...
public class PongThread extends Thread {
    ...
    private static final double PHYS_MAX_BOUNCE_ANGLE = 5 * Math.PI / 18; // 50 degrees in radians
     ...
     * save data per frame
     */
    ArrayList<GameFrameData> data = new ArrayList<GameFrameData>();
    DataPreparationListener dataPreparationListener;
    ...
    private void updatePhysics() {
        ...
        if(frame>=900 && !dataSended)
        {
            dataSended = true;
            dataPreparationListener.setData(data);
        }
    ...
    private void doAI() {
        ...
        if(AI_TYPE == 1) {
            ...
            if (mComputerPlayer.bounds.top > mBall.cy) {
                ...
                // save the move data
                data.add(new GameFrameData(mComputerPlayer.bounds.top, mComputerPlayer.paddleHeight, mBall.cy, mComputerPlayer.bounds.top - PHYS_PADDLE_SPEED));
                ++frame;
                ...
                System.out.println(mComputerPlayer.bounds.top - PHYS_PADDLE_SPEED);
        ...
        {
            X[0] =  mBall.cy;
            X[1] = mComputerPlayer.bounds.top;
            X[2] =  mComputerPlayer.paddleHeight;

            movePlayer(mComputerPlayer,
                    mComputerPlayer.bounds.left,
                    mlp.calculate(W, X,  layers,  amountInLayers)[0]);

            System.out.println(mlp.calculate(W, X,  layers,  amountInLayers)[0]);
        }
    }
    ...
    public void setDataListener(DataPreparationListener dataPreparationListener)
    {
        this.dataPreparationListener = dataPreparationListener;
    }
